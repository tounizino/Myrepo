...
class CGT_Frontend {
	...
	public static function render_platforms( $atts ) {
		...
		if ( $settings['group_platforms'] && $group_by === 'availability' ) {
			...
			$output  = self::render_platform_group( $available, esc_html__( 'Available Platforms', 'cloud-gaming-tracker' ), $settings, true );
			$output .= self::render_platform_group( $unavailable, esc_html__( 'Not Available', 'cloud-gaming-tracker' ), $settings, false );
		} else {
	...
	private static function render_platform_card( $platform, $settings, $is_available = null ) {
		...
		// For unavailable platforms, use a placeholder URL or custom unavailable CTA
		if ( ! $is_available ) {
			$cta_text = esc_html__( 'Coming Soon', 'cloud-gaming-tracker' );
